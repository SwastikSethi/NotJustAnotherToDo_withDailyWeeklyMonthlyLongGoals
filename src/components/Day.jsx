...
const Day = () => {
    ...
    useEffect(() => {
        let todostring = localStorage.getItem("todos-daily")
        if (todostring) {
            let oldtodo = JSON.parse(localStorage.getItem("todos-daily"));
            setTodos(oldtodo);
            console.log(todos);
        }
    ...
    const saveToLS = () => {
        localStorage.setItem("todos-daily", JSON.stringify(todos));
    }
    ...
    return (
        <>
            <div className="container flex flex-col gap-5 items-center rounded-xl bg-slate-700 text-white px-10 min-h-[80vh] md:w-[80vw] absolute md:left-[10vw]">
                ...
                <input onChange={toggleFinished} type="checkbox" checked={showFinished} class="w-5 justify-center h-5 rounded border-gray-300 focus:ring-indigo-500 checked:bg-indigo-500 checked:border-transparent"/> Show Finished Tasks
                ...
                <div className="todos flex flex-col  ">
                    ...
                    {todos.map(item => {
                        ...
                        return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex my-2 w-1/2 justify-between ">
                            <div className='flex place-items-center gap-2 min-w-[500px]'>
