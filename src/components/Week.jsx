...
const Week = () => {
    ...
    useEffect(() => {
        let todostring = localStorage.getItem("todos-weekly")
        if (todostring) {
            let oldtodo = JSON.parse(localStorage.getItem("todos-weekly"));
            setTodos(oldtodo);
            // console.log(todos);
        }
                ...
                <div className="todos flex flex-col  ">
                    ...
                    {todos.map(item => {
                        ...
                        return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex my-2 w-1/2 justify-between ">
                            ...
                            <input name={item.id} type="checkbox" onChange={handleComplete} checked={item.isCompleted} id={item.id} class="w-4 justify-center h-4 rounded border-gray-300 focus:ring-indigo-500 checked:bg-indigo-500 checked:border-transparent"/>
                                {/* svg */}
